...
local ignore_aid_observer = false
...
local function is_original_audio(track)
	...
	return lang == "" or not (lang:match("^ru") ~= nil or lang:match("^en") ~= nil)
end
...
local function is_english_audio(track)
	...
	return (track.lang or ""):lower():match("^en") ~= nil
end
...
local function is_russian_audio(track)
	...
	local lang = (track.lang or ""):lower()
	return lang:match("^ru") ~= nil or lang:match("^rus") ~= nil
end
...
local function is_full_russian_sub(sub)
	...
	if lang ~= "" and not (lang:match("^ru") or lang:match("^rus")) then
		return false
	end
	...
	local is_ru_lang = lang:match("^ru") or lang:match("^rus")
	if is_ru_lang then
		return true
	end
	...
	local priorities = ru_subs
			and {
				...
				function(t)
					...
					return not (lang:match("^ru") or lang:match("^en"))
